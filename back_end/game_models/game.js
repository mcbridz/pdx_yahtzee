...
gameSchema.methods.removePlayer = function (user) {
  ...
  let newList = this.users.filter(
    (playerObj) => !user._id.equals(playerObj.id)
  );
  ...
  // console.log(newList)
  this.users = newList;
  const newCardsArr = this.scoreCards.filter(
    (scoreCard) => user.username.trim() != scoreCard.player.trim()
  );
...
gameSchema.methods.performTasks = async function (taskObj) {
  ...
  taskObj.tasks.forEach(async (subTaskObj) => {
    await ScoreCard.prototype.$__schema.methods[subTaskObj.task].call(
      scoreCard,
      subTaskObj.data,
      taskObj,
      this.room
