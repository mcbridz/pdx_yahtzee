...
function App() {
  ...
  const testCreateGame = () => {
    socket.emit("createGame", {
      public: true,
      playerList: [
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2MGJlZDhlMWIyNjUzOTVkYjAwNjY3ZjUiLCJpYXQiOjE2MjMyOTI4MDl9.I8SWHgeMHPjATaQqGgZnmXxmkDZ343blVBk-U_wuysc",
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2MGJlZDkwNzcwNGUxZDZiYTBjMjJjZTEiLCJpYXQiOjE2MjMyOTI4MzV9.Jr88pvQi9iAm365QXAD_HoBayuOas-QXlkrB7mC7zHQ",
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2MGMxNzljZjY3YmY5NjZkNTQ2MjcyMDgiLCJpYXQiOjE2MjMyOTI4NjZ9.rULhPWRWsS-jnRFxNFX88Rq7ncLGN5RQ-p4H4oCMiF0",
      ],
    });
  };
  const testStartGame = () => {
  ...
  const player5 = {
    player:
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2MGMyYzU1MDBkZjE0NjRjNjQyZDFmZjUiLCJpYXQiOjE2MjMzNzcyNDJ9.0lUfCmaEdB8MfFXStH0UOirOHeq_qo9zy7-w6M_1O2A",
  };
  ...
  const order1 = {
    tasks: [
      { task: "markAces", data: 5 },
      { task: "markFourOfAKind", data: null },
    ],
  };
  const order2 = {
    tasks: [
      { task: "markTwos", data: 3 },
      { task: "markThreeOfAKind", data: null },
    ],
  };
  const order3 = {
    tasks: [
      { task: "markThrees", data: 2 },
      { task: "markFullHouse", data: null },
    ],
  };
  const order4 = {
    tasks: [
      { task: "markFours", data: 5 },
      { task: "markYahtzee", data: null },
    ],
  };
  const order5 = {
    tasks: [{ task: "markFives", data: 1 }],
  };
  const order6 = {
    tasks: [{ task: "markSixes", data: 3 }],
  };
  const order7 = {
    tasks: [{ task: "markYahtzee", data: null }],
  };
  const order8 = {
    tasks: [{ task: "markSmStraight", data: null }],
  };
  const order9 = {
    tasks: [{ task: "markLgStraight", data: null }],
  };
  ...
  return (
    <div className="App">
      ...
      {dice.map((die, index) => {
        return (
          <div key={index}>
            <label htmlFor={`${diceRef[index]}Die`}>
              {`#${diceRef[index]} Die`}{" "}
            </label>
            <input
              type="text"
              disabled
              name={`${diceRef[index]}Die`}
              value={die}
            />
          </div>
        );
      ...
      <ul name="players">
        {game.users.map((playerObj, key) => {
          return (
            <li key={key}>
              id: {playerObj.id} username: {playerObj.username}
            </li>
          );
        })}
      </ul>
      ...
      <input
        value={!game.started ? "" : game.currentPlayer.username}
        disabled
        name="currentPlayer"
      />
      ...
      />
      <label htmlFor="upperSectionTotalBonus">
        Upper Section Total + Bonus{" "}
      </label>
      <input
        value={scoreCard.bonus + scoreCard.upperSectionTotal}
        disabled
      ...
      <div>
        ...
        <button onClick={testRemovePlayer(player5)}>
          {" "}
          3. Remove 'player5'{" "}
        </button>
